...
// в 16-битный моно PCM WAV 16kHz через ffmpeg. Для гарантированной
// валидности RIFF-заголовков (с указанием точной длины фрагмента) вывод
// записывается во временный файл и сразу же удаляется после чтения.
func ConvertToWAV16k(ctx context.Context, src io.Reader) ([]byte, error) {
	if !HasFFmpeg() {
