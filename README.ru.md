...
- **Разговорный режим (по умолчанию)**:
  ...
  - Работает для любого агента (`agy`, `claude`) и в любом режиме (`cli`, `api`); переключение `/agent` или `/mode` посреди разговора не теряет нить.
...
- **Режимы выполнения CLI и API (`/mode [cli|api]`)**:
...
| | `cli` | `api` |
|---|---|---|
| **agy** | своя сессия агента (`--conversation`) | история диалога проигрывается из базы бота |
| **claude** | своя сессия агента (`--resume`) | история диалога проигрывается из базы бота |
...
| `/mode [cli|api]` | Переключить режим выполнения (локальный CLI или прямой API с файловыми инструментами). | `/mode api` |
