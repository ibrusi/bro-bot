...
| `WHISPER_PROMPT` | Нет | словарь IT-терминов | Контекстная подсказка и доменный словарь Whisper. Автоматически выбирается под язык `WHISPER_LANGUAGE` (`en` или `ru`). Для отключения укажите `none`. |
...
WHISPER_MODEL=small
WHISPER_LANGUAGE=en
WHISPER_PROMPT="дебаг режим, код, коммит, пулл реквест, git, бот, деплой, статус, таски, логи, фикс, ревью, билд, релиз, debug mode, PR"
