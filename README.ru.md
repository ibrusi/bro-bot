...
В корне репозитория подготовлен `Makefile` для автоматизации развертывания под Linux (Ubuntu/Debian) на сервере:

```bash
# Запуск полного развертывания (выполнять под root)
sudo make install
```
...
- `make help` — справка по всем доступным целям;
