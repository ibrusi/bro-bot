...
| `AUTO_CONTINUE_MAX` | Нет | `2` | Сколько раз подряд автоматически продолжать шаг, если агент завершил ход, не дождавшись своих фоновых команд (agy пишет `terminating N background task(s) on exit`). Продолжение идёт в той же сессии агента. Когда лимит исчерпан, задача ставится на паузу с кнопкой «Продолжить», а не объявляется завершённой. `0` отключает автопродолжение. |
