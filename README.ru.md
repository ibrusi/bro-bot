...
> - **Автономное выполнение команд**: `bro-bot` предоставляет AI-агентам (`agy`, `claude`) доступ к терминалу и файловой системе с повышенными правами (`--dangerously-skip-permissions`). Агенты могут выполнять shell-команды, редактировать код, устанавливать зависимости и пушить изменения в Git.
> - **Контроль доступа**: Доступ к боту строго ограничен параметром `TELEGRAM_ADMIN_ID`. Не запускайте бота под учетной записью `root`. Всегда проверяйте созданные агентом коммиты и Pull Request перед их слиянием.
> - **Квоты и расходы**: Автономные агенты генерируют множественные запросы к LLM API (Google Gemini, Anthropic Claude) и расходуют токены контекста и рассуждения. Контролируйте лимиты и баланс квот.
> - **Отказ от ответственности (AS IS)**: Данное программное обеспечение предоставляется «как есть», без каких-либо явных или подразумеваемых гарантий. Разработчики не несут ответственности за возможную потерю данных, непредвиденные системные сбои или финансовые затраты на API.
...
- **Широкие полномочия агентов**: Сервис запускает агентные CLI (`agy` и `claude`) с флагом `--dangerously-skip-permissions`. Это означает, что автономные агенты могут без ручного подтверждения каждого действия создавать, изменять и удалять файлы, запускать shell-команды, устанавливать пакеты и управлять Git-репозиториями. Всегда запускайте бота от имени непривилегированного пользователя (например, `deploy`) и ограничивайте права `sudoers`.
- **Контроль за изменениями**: Хотя бот автоматически проверяет код тестами и линтерами (согласно регламенту `AGENT.md`), ответственность за финальную валидацию изменений, коммитов и созданных Pull Request лежит на разработчике.
- **Потребление токенов и финансы**: Работа с продвинутыми языковыми моделями (Google Gemini 3.1 Pro/Flash, Claude 3.7 Sonnet/Opus) сопряжена с расходом токенов контекста и рассуждений (thinking). Регулярно проверяйте метрики через `/usage` и `/tokens`, чтобы избежать неожиданных расходов.
- **Отказ от ответственности (AS IS)**: Данный проект является открытым и распространяется на условиях лицензии MIT «как есть», без каких-либо гарантий. Авторы проекта не несут ответственности за повреждение системы, утрату данных, блокировки учетных записей или финансовые издержки, возникшие в результате использования бота.
