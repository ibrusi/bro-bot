# 🤖 bro-bot (Bro Bot)

[![Go Version](https://img.shields.io/badge/Go-1.23+-00ADD8?style=flat&logo=go)](https://golang.org/)
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)
[![Platform](https://img.shields.io/badge/platform-Linux-lightgrey.svg)](https://kernel.org)

[English](README.md) | **Русский**

**bro-bot** — это автономный сервис-мост на Go, соединяющий **Telegram** с агентной средой **Google Antigravity CLI (`agy`)** и **Claude Code CLI (`claude`)**. 

Бот позволяет разработчику или команде управлять пулом проектов, ставить задачи на кодинг и рефакторинг, утверждать интерактивные планы реализации, отвечать на уточняющие вопросы агента, контролировать лимиты токенов и потребление ресурсов сервера — не покидая Telegram.

<a id="дисклеймер"></a>
> [!WARNING]
> **Дисклеймер о безопасности и использовании**:
> - **Автономное выполнение команд и широкие полномочия**: `bro-bot` запускает AI-агентов (`agy`, `claude`) с повышенными привилегиями (`--dangerously-skip-permissions`). Агенты могут в автономном режиме без подтверждения каждого действия выполнять shell-команды, создавать, изменять и удалять файлы, устанавливать зависимости и управлять Git-репозиториями. Всегда запускайте сервис от имени выделенного непривилегированного пользователя (например, `deploy`), ограничивайте правила `sudoers` и строго контролируйте доступ с помощью `TELEGRAM_ADMIN_ID` (не запускайте бота под `root`).
> - **Риск блокировки аккаунтов при использовании подписок (режим CLI)**: Использование CLI-агентов, авторизованных через персональные подписки (Claude Pro/Team/Max, Google One AI Premium/Gemini Advanced), для автоматизации через бота в режиме `cli` нарушает условия использования (Terms of Service) соответствующих сервисов (запрет на автоматизацию интерактивных интерфейсов и CLI без официального API). Вы используете режим `cli` исключительно на свой страх и риск — за это провайдеры могут заблокировать вашу учетную запись. Режим работы через официальные API-ключи (`api`) полностью соответствует правилам платформ и к блокировке аккаунта не приведет.
> - **Контроль за изменениями**: Хотя бот и агенты следуют регламенту `AGENTS.md` (запуск линтеров и тестов), ответственность за финальную проверку и валидацию сгенерированного кода, коммитов и созданных Pull Request перед их слиянием в рабочие ветки целиком лежит на разработчике.
> - **Квоты, токены и расходы на API**: Работа с продвинутыми языковыми моделями (Google Gemini, Anthropic Claude) сопровождается активным расходом токенов контекста, генерации и рассуждения (thinking). Регулярно отслеживайте лимиты квот и баланс в дашбордах провайдеров, а также с помощью команд бота `/usage` и `/tokens`, чтобы избежать непредвиденных финансовых расходов.
> - **Отказ от ответственности (AS IS)**: Данное программное обеспечение распространяется на условиях лицензии MIT «как есть», без каких-либо явных или подразумеваемых гарантий. Авторы и контрибьюторы проекта не несут ответственности за возможную потерю данных, системные сбои, блокировки учетных записей или любые финансовые издержки, возникшие в результате использования бота.


---

## 📑 Содержание

- [✨ Ключевые возможности](#-ключевые-возможности)
- [🏗 Архитектура решения](#-архитектура-решения)
- [📦 Системные требования](#-системные-требования)
- [🚀 Установка и сборка](#-установка-и-сборка)
- [⚙️ Конфигурация (.env)](#️-конфигурация-env)
- [🛠 Настройка Systemd и автозапуска](#-настройка-systemd-и-автозапуска)
- [💬 Справочник команд бота](#-справочник-команд-бота)
- [🌍 Язык интерфейса и локализация](#-язык-интерфейса-и-локализация)
- [📋 Регламент работы агента (AGENTS.md)](#-регламент-работы-агента-agentmd)
- [🧪 Разработка и тестирование](#-разработка-и-тестирование)
- [🔒 Безопасность](#-безопасность)
- [📄 Лицензия](#-лицензия)

---

## ✨ Ключевые возможности

- **Управление проектами и репозиториями**:
  - Быстрое переключение активного рабочего проекта (`/projects`, `/use <имя>`).
  - Клонирование Git-репозиториев по SSH или HTTPS на лету (`/clone <url> [имя]`) с автоматической подстановкой шаблона инструкций `AGENTS.md`.
- **Разговорный режим (по умолчанию)**:
  - Обычное сообщение — это диалог с агентом по активному проекту: контекст сохраняется между сообщениями, `/resume` больше не нужен.
  - Если сообщение похоже на запрос на изменение кода, бот не выполняет его молча, а предлагает кнопками: ответить в чате, составить план или создать задачу.
  - Работает для любого агента (`agy`, `claude`) и в любом режиме (`mcp`, `cli`, `api`); переключение `/agent` или `/mode` посреди разговора не теряет нить.
  - В чате агент только читает код и отвечает: изменения файлов, ветки, коммиты и PR — только через задачу.
  - Управление: `/chat` (статус), `/chat <вопрос>`, `/chat new`, `/chat stop`, `/chatmode [on|off]`.
- **Интеллектуальный пайплайн задач**:
  - Поддержка независимых очередей задач по проектам.
  - Режим предварительного планирования (`/plan <задача>`, `/planmode [on|off]`): агент сначала составляет и аргументирует архитектурный план, запрашивает подтверждение через интерактивные inline-кнопки (`/approve`, `/confirm`) и только потом приступает к коду.
  - Пауза и возобновление (`/pause [id]`, `/resume [id] [ответ]`).
  - Живой стриминг статуса выполнения, последних логов, компактных ресурсов системы (Bot, `agy`, `claude`, `whisper-server`) и текущего шага (`/status [id]`, `/tasks`).
  - Возможность отмены зависших или неактуальных задач (`/cancel [id]`).
- **Интерактивные диалоги и вопросы агента (`ask_question`)**:
  - Если агенту требуются уточнения, бот формирует опрос с кнопками вариантов выбора прямо в Telegram.
  - Возможность ответить обычным текстом, через Reply на сообщение задачи или командой `/add [id] <текст>`.
  - Автоматический таймаут (`QUESTION_TIMEOUT`): если ответа нет в течение заданного времени, задача переводится на паузу и разблокирует очередь для других задач.
- **Поддержка моделей и агентов**:
  - Автоматическое обнаружение доступных моделей из CLI (`agy models`) с кэшированием на диске и запасными механизмами.
  - Поддержка удобных алиасов: `default`, `flash`, `flash-high`, `flash-low`, `pro`, `pro-low`, `sonnet`, `opus`, `oss`.
  - Мгновенная смена активной модели (`/models`, `/model <имя|алиас>`).
- **Мониторинг квот, токенов и контекста**:
  - `/usage` (or `/limits`) — актуальные остатки квот и баланс кредитов аккаунта Antigravity (`agy /quota`, `agy /credits`).
  - `/tokens` (or `/stats`) — детальная статистика расхода токенов текущей/завершенной задачи: входные (Input), выходные (Output), токены рассуждения (Thinking), чтение из кэша (Cache Read) и процент кэш-хитов (Cache Hit Rate).
  - `/context [id]` — наглядная диаграмма заполнения контекстного окна модели (исходные инструкции, история диалога, вывод инструментов).
  - `/top` (или `/ps`, `/resources`) — мониторинг нагрузки хоста в реальном времени: CPU, RAM, свободное место на диске, Load Average, метрики cgroup службы systemd, процессы агентов `agy` и `claude`, а также сервера распознавания речи `whisper-server`.
- **Режимы выполнения: MCP, CLI и API (`/mode [mcp|cli|api]`)**:
  - `mcp` (по умолчанию): запуск агентов с интеграцией через встроенный сервер Model Context Protocol (MCP) по stdio JSON-RPC 2.0 (`claude` через `--mcp-config`, `agy` через `mcp_config.json`) с предоставлением разрешенных инструментов (`telegram_send_message`, `ask_user`, `report_progress`).
  - `cli`: запуск локальных утилит `agy` и `claude` через PTY.
  - `api`: прямое взаимодействие через официальные API (Google Gemini API и Anthropic Messages API) со встроенными инструментами файлового доступа (`read_file`, `write_file`, `edit_file`, `list_dir`, `run_command`).
  - Безопасное разграничение: в диалоге (`/chat`) активны только инструменты чтения (read-only), а изменение файлов и команды запускаются только в задачах пайплайна. Пути изолированы в пределах директории проекта.
- **Многоязычный интерфейс**:
  - Каждый ответ бота, кнопка, описание команды и промпт агенту берутся из каталога сообщений — английский и русский идут в комплекте.
  - Пока язык не выбран, бот отвечает по-английски; выбор делается командой `/language` и переживает перезапуск.
  - Новый язык — это один JSON-файл в `internal/i18n/locales/`, без правок кода на Go (см. [Язык интерфейса и локализация](#-язык-интерфейса-и-локализация)).
- **Распознавание голосовых сообщений (Whisper)**:
  - Голосовые заметки и аудиофайлы из Telegram автоматически распознаются в текст через `whisper-server`.
  - Аудио конвертируется через `ffmpeg` в 16-битный моно WAV 16kHz перед отправкой, гарантируя совместимость с любыми сборками Whisper.
  - Распознанный текст цитируется в чат (`🗣 «...»`) с мгновенным выполнением: команды (`/status`, `/plan` и т.д.) выполняются напрямую, а обычная речь направляется в диалог или создание задачи.
  - В режиме отладки (`DEBUG=true`) к цитате прикрепляются метрики производительности: время распознавания STT, длительность аудио и RTF (Real-Time Factor).
  - Распознавание речи по умолчанию работает на английском языке (`en`). Для переключения языка, например на русский, укажите `WHISPER_LANGUAGE=ru` в файле `.env`.
  - Быстрая компиляция сервера и настройка службы одной командой: `sudo make install-whisper`.
- **Горячее обновление и пересборка (CI/CD прямо в боте)**:
  - `/restart` — корректный неблокирующий перезапуск службы systemd.
  - `/rebuild [ветка] [--pull] [--force]` — обновление кода бота из Git, компиляция нового бинарника, атомарная замена файла и перезапуск с отправкой уведомления об успешном старте и хэше коммита.

---

## 🏗 Архитектура решения

```
┌──────────────────────────────────────────────────────────┐
│              Telegram Client (или другой мессенджер)     │
└────────────────────────────┬─────────────────────────────┘
                             │  HTTPS / Long Polling
                             ▼
┌──────────────────────────────────────────────────────────┐
│  internal/adapters/telegram/  ──► ports.Transport impl.   │
│  (единственный пакет, знающий про gopkg.in/telebot.v3;    │
│   новый мессенджер = новый пакет здесь же, по образцу)    │
└────────────────────────────┬─────────────────────────────┘
                             │ internal/ports (Messenger, Transport, Session)
                             ▼
┌──────────────────────────────────────────────────────────┐
│                 bro-bot (Go Executable)                  │
│                                                          │
│  internal/handlers/  ──► Команды и колбэки (мессенджеро-  │
│                           независимо, через ports.Session)│
│  internal/domain/    ──► Task Queue, Sessions & Tokens   │
│  internal/agents/    ──► Реестр агентов и роутер режимов │
│  internal/storage/   ──► SQLite Persistence & Migrations │
│  internal/models/    ──► Model Registry & Aliases        │
│  internal/mcp/       ──► Встроенный stdio MCP-сервер     │
│  internal/tools/     ──► Файловые инструменты и shell    │
│  internal/i18n/      ──► Каталоги сообщений (locales/*.json)│
│  internal/system/    ──► Hot-Rebuild, Systemd & Top/PS   │
│  internal/utils/     ──► Rich-текст и Markdown парсер    │
└───────┬───────────────────┬────────────────────┬─────────┘
        │ Режим: mcp        │ Режим: cli         │ Режим: api
        │ (по умолчанию)    │ (PTY)              │ (Прямой REST)
        ▼                   ▼                    ▼
┌────────────────┐  ┌────────────────┐  ┌────────────────┐
│ Claude / Agy c │  │ Процессы CLI   │  │ Клиенты        │
│ поддержкой MCP │  │ agy и claude   │  │ провайдеров:   │
│                │  │                │  │ • Gemini API   │
│ • bot mcp-serve│  │ • stream-json  │  │ • Anthropic    │
│ • JSON-RPC 2.0 │  │ • PTY-терминал │  │   Messages API │
│ • Каналы, опрос│  │ • Парсинг      │  │ • Инструменты  │
│   и прогресс   │  │   событий      │  │   чтения/записи│
└───────┬────────┘  └───────┬────────┘  └───────┬────────┘
        │                   │                   │
        └───────────────────┼───────────────────┘
                            ▼
               ┌─────────────────────────┐
               │   Каталог проектов      │
               │   /home/deploy/projects │
               │   ├── project-1/        │
               │   └── project-2/        │
               └────────────┬────────────┘
                            │
                            ▼ WAL Mode
               ┌─────────────────────────┐
               │   База данных SQLite    │
               │   data/bot.db           │
               │   (Задачи, планы, логи) │
               └─────────────────────────┘
```

### Архитектура режимов выполнения

`bro-bot` абстрагирует выполнение задач через интерфейс `ports.Agent` и реестр `agents.Registry`, предоставляя три независимых режима работы:

1. **Режим MCP (`mcp`, по умолчанию)**:
   - Взаимодействие локальных CLI-агентов (`claude`, `agy`) через встроенный сервер Model Context Protocol (MCP), работающий по протоколу JSON-RPC 2.0 stdio через подкоманду `bot mcp-serve`.
   - `claude` подключается через флаг `--mcp-config` с поддержкой экспериментальных каналов (`claude/channel`) и автоматической очисткой временного конфигурационного файла после завершения процесса.
   - `agy` подключается через глобальную конфигурацию в `~/.gemini/config/mcp_config.json`.
   - Сервер предоставляет агентам разрешенные инструменты:
     - `telegram_send_message`: отправка сообщений и уведомлений в чат Telegram;
     - `ask_user`: интерактивный опрос пользователя с вариантами ответов;
     - `report_progress`: передача контрольных точек и прогресса выполнения задачи.

2. **Режим CLI (`cli`)**:
   - Запуск бинарников CLI (`agy`, `claude`) внутри изолированных псевдотерминалов (PTY) с флагами:
     `agy --dangerously-skip-permissions --print-timeout 30m --output-format stream-json [--conversation <id>] --model <model> -p <prompt>`
     `claude --dangerously-skip-permissions --print --output-format stream-json [--resume <id>] --model <model> -p <prompt>`
   - Бот считывает потоковый NDJSON-вывод `stream-json`, перехватывает фазы рассуждений (thinking), вызовы инструментов и события завершения, транслируя их в сообщения мессенджера в реальном времени.

3. **Режим API (`api`)**:
   - Прямой сетевой обмен через HTTP/REST с официальными API провайдеров:
     - Google Gemini API (`GEMINI_API_KEY`) через Google GenAI SDK;
     - Anthropic Messages API (`ANTHROPIC_API_KEY` / `CLAUDE_API_KEY`).
   - Встроенный движок инструментов работы с проектом (`internal/tools`):
     - Безопасный доступ к файлам (`read_file`, `write_file`, `edit_file`, `list_dir`) со строгой изоляцией внутри директории активного проекта.
     - Запуск команд shell (`run_command`) при реализации задач.
     - Разграничение прав: в разговорном режиме (`/chat`) доступно только безопасное чтение, а изменение файлов и запуск команд разрешены только в задачах пайплайна (`/new`, `/plan`).

### Абстракция мессенджера

Весь код бота (`internal/handlers`, `internal/domain`, `internal/system`) общается с мессенджером только через интерфейсы `ports.Messenger` (отправка/редактирование сообщений, документы, инлайн-кнопки) и `ports.Transport` (приём команд, текста и колбэков). Единственная реализация на сегодня — `internal/adapters/telegram`, и только этот пакет импортирует `gopkg.in/telebot.v3`. Переменная окружения `MESSENGER` (по умолчанию `telegram`) выбирает адаптер в `cmd/bot/main.go`. Чтобы добавить новый мессенджер, нужно реализовать `ports.Transport` в новом пакете `internal/adapters/<name>/` и зарегистрировать его в `cmd/bot/main.go` — код хендлеров при этом не меняется.

---

## 📦 Системные требования

- **Операционная система**: Linux (Ubuntu 22.04+, Debian 11+, RHEL/Rocky 9+).
- **Go**: Версия `1.23` или выше.
- **Git**: Установленный и настроенный Git.
- **GitHub CLI (`gh`)**: Установлен и авторизован (`gh auth login`) для автоматического создания Pull Request.
- **SSH-ключи**: Добавлены на GitHub / GitLab для клонирования и пуша веток проектов.
- **Google Antigravity CLI (`agy`)**: Установлен, добавлен в `$PATH` пользователя (например, `/home/deploy/.local/bin/agy`) и авторизован.
- **Telegram Bot Token**: Создан через [@BotFather](https://t.me/BotFather).
- **Telegram User ID**: Числовой ID вашего аккаунта (узнать можно у [@userinfobot](https://t.me/userinfobot)).

---

## 🚀 Установка и сборка

### Вариант A. Автоматическое развертывание через Makefile (рекомендуется для чистых серверов)

В корне репозитория подготовлен `Makefile` для автоматизации развертывания под Linux (Ubuntu/Debian) на сервере с поддержкой двух языков сообщений (английский по умолчанию, с возможностью переключения на русский):

```bash
# Запуск полного развертывания (по умолчанию на английском, выполнять под root)
sudo make install

# Запуск полного развертывания с сообщениями на русском языке
sudo make install LANG=ru
# или: sudo make install L=ru
```

Команда `make install` последовательно выполняет:
1. **`step1-user`**: Устанавливает базовые системные пакеты (`git`, `curl`, `wget`, `build-essential`, `sudo`, `python3`, `python3-pip`, `ffmpeg`), создает системного пользователя `deploy` с беспарольным `sudo` и подготавливает рабочие директории (`~/.local/bin`, `~/projects`).
2. **`step2-agents`** (или **`step2-agy`**): Проверяет и восстанавливает `agy`, устанавливает Claude Code CLI и настраивает подключение MCP-сервера `bro_bot` для обоих агентов.
3. **`step3-service`**: Создает systemd unit `/etc/systemd/system/bro-bot.service` с автоперезапуском при сбоях и загрузкой `.env`, регистрирует и активирует службу в автозагрузке.
4. **`step4-clone-build`**: Проверяет наличие Go 1.23+ (при необходимости скачивает и устанавливает), клонирует или обновляет репозиторий в `/home/deploy/bro-bot`, подготавливает `.env` из `.env.example` и компилирует бинарник `bot`.
5. **`step5-start`**: Перезапускает службу `bro-bot.service`, проверяет статус активности и выводит текущий статус.

После выполнения развертывания укажите токен и конфигурацию в `/home/deploy/bro-bot/.env` и перезапустите службу:
```bash
sudo systemctl restart bro-bot.service
```

#### Полезные команды Makefile для разработки:
- `make help` — справка по всем доступным целям на английском языке (`make help LANG=ru` для вывода на русском);
- `sudo make install-ffmpeg` — установка `ffmpeg` (необходим для конвертации голосовых сообщений);
- `sudo make install-whisper` — компиляция сервера `whisper.cpp`, скачивание моделей `base-q5_1` и Silero VAD `silero-v6.2.0`, регистрация службы `whisper-server.service`;
- `make build` — компиляция исполняемого файла `./bot` (`go build -o bot ./cmd/bot`);
- `make test` — запуск всех тестов проекта (`go test ./...`);
- `make run` — запуск бота напрямую из исходников (`go run ./cmd/bot`);
- `make clean` — удаление скомпилированного бинарника `bot`.

#### Выбор языка сообщений в Makefile:
По умолчанию сообщения выводятся на английском языке (`en`). Для вывода на русском языке передайте параметр `LANG=ru` или короткий `L=ru`:
```bash
make help            # На английском языке (по умолчанию)
make help LANG=ru    # На русском языке (или L=ru)
sudo make install LANG=ru
```

---

### Вариант B. Ручная установка и сборка

#### 1. Клонирование репозитория

```bash
# Рекомендуемый путь установки: /home/deploy/bro-bot
cd /home/deploy
git clone git@github.com:ibrusi/bro-bot.git
cd bro-bot
```

#### 2. Создание каталога проектов

Создайте каталог, где будут располагаться управляемые ботом проекты (по умолчанию `/home/deploy/projects`):

```bash
mkdir -p /home/deploy/projects
```

#### 3. Сборка проекта

Загрузите зависимости Go и соберите исполняемый файл:

```bash
go mod download
go build -o bot ./cmd/bot
```

В каталоге появится исполняемый файл `bot`.

---

## ⚙️ Конфигурация (.env)

Создайте файл `.env` в корне каталога бота на основе примера `.env.example`:

```bash
cp .env.example .env
nano .env
```

### Таблица переменных окружения

| Переменная | Обязательная | Значение по умолчанию | Описание |
|---|:---:|:---:|---|
| `MESSENGER` | Нет | `telegram` | Мессенджер-адаптер из `internal/adapters/`, который использует бот (см. [Абстракция мессенджера](#абстракция-мессенджера)). Сейчас поддерживается только `telegram`. |
| `TELEGRAM_BOT_TOKEN` | **Да** | — | Токен Telegram-бота, полученный от [@BotFather](https://t.me/BotFather). |
| `TELEGRAM_ADMIN_ID` | **Да** | — | Числовой Telegram ID администратора. Бот принимает команды **только** от этого пользователя, все остальные игнорируются. |
| `PROJECTS_ROOT` | **Да** | — | Абсолютный путь к каталогу, где хранятся репозитории рабочих проектов. |
| `SCRIPTS_DIR` | Нет | `scripts` в `BOT_DIR` | Путь к каталогу, где хранятся пользовательские скрипты для команды `/script`. |
| `DEFAULT_PROJECT` | Нет | Первая папка в `PROJECTS_ROOT` | Имя проекта (подпапки в `PROJECTS_ROOT`), выбираемого активным при старте. Если не задано, бот использует первую найденную папку. |
| `DEFAULT_MODEL` | **Да** | — | Модель по умолчанию для `agy` (например `gemini-3.1-pro-high`, `flash`, `sonnet`, `opus`). |
| `GEMINI_API_KEY` | Нет | — | Ключ Google AI Studio для работы агента `agy` в режиме `api` (`/mode api`). Без него режим `api` недоступен. |
| `GEMINI_API_MODEL` | Нет | Автовыбор | Явное имя модели Gemini API для режима `api` (например `gemini-2.5-flash`). Если не задано, модель подбирается по списку доступных моделей: берётся младшее семейство из старших (flash) в максимальной доступной версии. |
| `ANTHROPIC_API_KEY` | Нет | — | Ключ Anthropic для работы агента `claude` в режиме `api` (`/mode api`). Без него (и без `CLAUDE_API_KEY`) режим `api` у `claude` недоступен. |
| `CLAUDE_API_KEY` | Нет | — | Устаревшее имя той же переменной: используется, если `ANTHROPIC_API_KEY` не задан. |
| `CLAUDE_API_MODEL` | Нет | Автовыбор | Явное имя модели Claude API для режима `api` (например `claude-sonnet-5`). Если не задано, модель подбирается по списку `/v1/models`: берётся младшее семейство из старших (sonnet) в максимальной доступной версии. Снятые с обслуживания модели автоматически заменяются живыми того же семейства. |
| `QUESTION_TIMEOUT` | **Да** | — | Таймаут ожидания ответа на вопрос агента (`ask_question`). Поддерживает форматы `15m`, `300s`, `1h` или число секунд. По истечении задача ставится на паузу. |
| `STEP_TIMEOUT` | Нет | `30m` | Таймаут выполнения одного шага агента `agy` (`--print-timeout`). Поддерживает форматы `30m`, `1h`, `1800s` или секунды. При превышении задача ставится на паузу с сохранением сессии `agy`. |
| `AUTO_CONTINUE_MAX` | Нет | `2` | Сколько раз подряд автоматически продолжать шаг, если агент завершил ход, не дождавшись своих фоновых команд (agy пишет `root agent idle; waiting … background task(s)`, после чего агент не просыпается, и/или `terminating N background task(s) on exit`). Продолжение идёт в той же сессии агента. Когда лимит исчерпан, задача ставится на паузу с кнопкой «Продолжить», а не объявляется завершённой. `0` отключает автопродолжение. |
| `CHAT_TIMEOUT` | Нет | `5m` | Таймаут одного ответа в разговорном режиме (`/chat`). Поддерживает форматы `5m`, `300s` или число секунд. |
| `BOT_DIR` | Нет | Каталог исполняемого файла | Путь к исходному коду бота для выполнения команд `/rebuild` и сохранения маркера перезапуска. Если не задан, берётся каталог исполняемого файла — но только если рядом с ним лежит `go.mod`. Иначе бот не стартует и сообщает об этом. |
| `BOT_SERVICE_NAME` | **Да** | — | Имя службы systemd для перезапуска через команды `/restart` и `/rebuild`. |
| `SQLITE_DB_PATH` | Нет | `data/bot.db` в `BOT_DIR` | Путь к файлу базы данных SQLite для персистентного хранения задач, архитектурных планов, логов, метрик и настроек бота. |
| `WHISPER_SERVER_URL` | Нет | `http://127.0.0.1:8080/inference` (в `.env.example`) | HTTP-эндпоинт сервера распознавания речи Whisper (например, `whisper.cpp`, `faster-whisper`, OpenAI API). Поддерживает `/inference` и `/v1/audio/transcriptions` с автоматическим fallback при 404. Оставьте пустым для отключения распознавания речи. |
| `WHISPER_API_KEY` | Нет | — | Опциональный токен авторизации (Bearer token), если сервер Whisper требует аутентификации. |
| `WHISPER_MODEL` | Нет | `base-q5_1` | Имя модели, передаваемое серверу Whisper. |
| `WHISPER_LANGUAGE` | Нет | `en` | Код языка распознавания речи (`en`, `ru`, `auto` и др.). По умолчанию используется `en`. Для переключения языка, например на русский, необходимо изменить значение на `WHISPER_LANGUAGE=ru`. |
| `WHISPER_PROMPT` | Нет | — (пусто) | Контекстная подсказка и доменный словарь Whisper для снижения фонетических искажений терминов. Задаётся в `.env` (в `.env.example` предзаполнен словарь IT-терминов). Для отключения оставьте пустым или укажите `none`. |
| `WHISPER_TEMPERATURE` | Нет | `0.0` | Температура сэмплинга (`0.0` для детерминированного жадного декодирования без галлюцинаций). |
| `WHISPER_LOUDNORM` | Нет | `true` | Нормализация громкости через ffmpeg (`-af loudnorm`) для разборчивости тихого голоса и шёпота. Для отключения укажите `false`. |
| `WHISPER_TIMEOUT` | Нет | `60s` | Таймаут HTTP-запроса на распознавание аудио. Форматы: `60s`, `2m` или секунды. |
| `DEBUG` | Нет | `false` (пусто) | Режим отладки. При значении `true` или `1` в Telegram отображаются замеры скорости транскрибации (STT Latency, Audio Duration, RTF). |

### Пример `.env`

```dotenv
TELEGRAM_BOT_TOKEN=123456789:ABCdefGHIjklMNOpqrSTUvwxYZ
TELEGRAM_ADMIN_ID=123456789
PROJECTS_ROOT=/home/deploy/projects
DEFAULT_PROJECT=bro-bot
DEFAULT_MODEL=gemini-3.1-flash-high
QUESTION_TIMEOUT=15m
STEP_TIMEOUT=30m
AUTO_CONTINUE_MAX=2
CHAT_TIMEOUT=5m
BOT_DIR=/home/deploy/bro-bot
BOT_SERVICE_NAME=bro-bot.service
SQLITE_DB_PATH=data/bot.db
WHISPER_SERVER_URL=http://127.0.0.1:8080/inference
WHISPER_MODEL=base-q5_1
WHISPER_LANGUAGE=ru
WHISPER_PROMPT="дебаг, дебаг режим, debug, git, checkout, gitlab, github, docker ps, docker compose, gRPC, парсить, шёпот, распознавание, коммит, пулл реквест, PR, мердж, деплой, билд, прод, логи, статус, таски, фикс, VAD, STT"
WHISPER_TEMPERATURE=0.0
WHISPER_LOUDNORM=true
DEBUG=
```

---

## 🛠 Настройка Systemd и автозапуска

Для круглосуточной стабильной работы и корректного функционирования команд `/restart` и `/rebuild` рекомендуется настроить systemd-сервис.

### 1. Создание сервиса `/etc/systemd/system/bro-bot.service`

Создайте файл сервиса с помощью `sudo`:

```ini
[Unit]
Description=Telegram Agent Bridge Bot (Go)
After=network.target network-online.target
Wants=network-online.target

[Service]
Type=simple
User=deploy
Group=deploy
WorkingDirectory=/home/deploy/bro-bot

EnvironmentFile=/home/deploy/bro-bot/.env
# Убедитесь, что в PATH входят пути к go, agy и пользовательским бинарникам
Environment="PATH=/usr/local/go/bin:/home/deploy/go/bin:/home/deploy/.local/bin:/usr/bin:/bin"

ExecStart=/home/deploy/bro-bot/bot

Restart=always
RestartSec=5s

StartLimitIntervalSec=60
StartLimitBurst=5
TimeoutStopSec=15
KillMode=mixed

[Install]
WantedBy=multi-user.target
```

### 2. Настройка прав `sudoers` для перезапуска сервиса

Команды бота `/restart` и `/rebuild` выполняют `sudo systemctl restart bro-bot.service`. Чтобы бот мог перезапускать себя без запроса пароля, добавьте правило в sudoers:

```bash
sudo visudo -f /etc/sudoers.d/bro-bot
```

Добавьте строку (замените `deploy` на имя вашего пользователя):

```sudoers
deploy ALL=(ALL) NOPASSWD: /bin/systemctl restart bro-bot.service, /usr/bin/systemctl restart bro-bot.service
```

### 3. Запуск и проверка службы

```bash
# Перечитать конфигурацию systemd
sudo systemctl daemon-reload

# Включить автозапуск и запустить службу
sudo systemctl enable --now bro-bot.service

# Проверить статус
sudo systemctl status bro-bot.service

# Просмотр логов в реальном времени
journalctl -u bro-bot.service -f
```

---

## 💬 Справочник команд бота

Бот управляется текстовыми сообщениями и slash-командами в чате Telegram.

### 💬 Память разговора по агентам и режимам

История диалога хранится в базе бота и не зависит от агента и режима, а идентификатор сессии
агента свой у каждой пары «агент + режим»:

| | `mcp` (по умолчанию) | `cli` | `api` |
|---|---|---|---|
| **agy** | своя сессия агента с MCP-конфигурацией (`mcp_config.json`) | своя сессия агента (`--conversation`) | история диалога проигрывается из базы бота |
| **claude** | своя сессия агента с MCP-конфигурацией (`--mcp-config`) | своя сессия агента (`--resume`) | история диалога проигрывается из базы бота |

При переключении `/agent` или `/mode` новый агент получает краткий контекст предыдущих реплик,
поэтому разговор продолжается. Сбросить контекст: `/chat new`.

> ⚠️ Ограничение только на чтение в разговорном режиме — это инструкция в промпте агента, а не песочница.
> Если нужна гарантия, что код не изменится, используйте отдельную ветку или задачу.

### 📌 Управление задачами и планированием

| Команда | Описание | Пример |
|---|---|---|
| `сообщение` | В разговорном режиме (по умолчанию) — вопрос агенту с сохранением контекста; запрос на изменение кода бот предложит оформить планом или задачей. При `/chatmode off` — сразу создаёт задачу. | `Как устроен пайплайн задач?` |
| `/chat [вопрос\|new\|stop]` | Без аргументов — статус разговора (проект, агент, режим, вид памяти). С текстом — разовый вопрос в обход классификатора. `new` — начать разговор заново, `stop` — прервать текущий ответ. | `/chat покажи, где регистрируются команды` |
| `/chatmode [on\|off]` | Отвечать на обычные сообщения в чате (`on`, по умолчанию) или сразу создавать задачу (`off`). | `/chatmode off` |
| `/plan [проект] <текст>` | Запуск задачи в режиме предварительного планирования (агент исследует репозиторий, формирует план и ждет подтверждения). | `/plan Спроектируй архитектуру кэширования` |
| `/planmode [on\|off]` | Включение/выключение обязательного режима планирования для всех входящих задач. | `/planmode on` |
| `/approve [id]` (или `/confirm`) | Утвердить предложенный агентом план задачи и запустить реализацию. | `/approve 3` |
| `/tasks` | Список всех задач текущей сессии с их статусами и кнопками быстрого фокуса. | `/tasks` |
| `/task <id> [текст]` | Переключить контекст на конкретную задачу или отправить ей дополнение. | `/task 2 обнови только README` |
| `/add [id] <текст>` | Отправить уточнение или дополнение к выполняющейся задаче. | `/add 1 также напиши тесты на хэндлер` |
| `/new [проект] [агент] <текст>` | Создать новую задачу явно (с возможностью указать проект и/или агента отличных от текущих). | `/new my-service claude оптимизируй SQL запросы` |
| `/pause [id]` | Приостановить выполнение задачи (разблокирует очередь проекта для других задач). | `/pause 1` |
| `/resume [id] [ответ]` | Возобновить выполнение приостановленной задачи (по таймауту вопроса или таймауту шага `agy`) с сохранением сессии и контекста. | `/resume 1 используй второй вариант` |
| `/retry [id]` | Перезапустить задачу с чистой сессией `agy` (сбросить `conversation_id`, если нужно начать выполнение с чистого листа). | `/retry 2` |
| `/status [id]` | Подробный статус задачи: модель, сессия `agy`, затраченное время, последние логи, очередь правок. | `/status 2` |
| `/history [id]` | Показать историю сообщений и логов агента/пользователя в рамках сессии задачи. | `/history 2` |
| `/cancel [id]` | Прервать и отменить выполнение задачи. | `/cancel 2` |

> 💡 **Совет:** Вы можете отвечать на любые сообщения бота о задаче через стандартный **Reply** (ответить на сообщение) в Telegram — бот автоматически направит ваш ответ нужной задаче!

---

### 📂 Управление проектами и репозиториями

| Команда | Описание | Пример |
|---|---|---|
| `/projects` | Показать список доступных проектов в `PROJECTS_ROOT` с отметкой текущего активного. | `/projects` |
| `/use <имя>` | Переключить текущий активный проект. | `/use my-backend-api` |
| `/clone <url> [имя]` | Склонировать репозиторий по SSH или HTTPS в каталог `PROJECTS_ROOT`. | `/clone git@github.com:org/repo.git my-repo` |

---

### 🧠 Модели, квоты и ресурсы

| Команда | Описание | Пример |
|---|---|---|
| `/models` | Список всех доступных моделей из Antigravity CLI с их описанием и алиасами. | `/models` |
| `/model [имя]` | Переключить активную модель для будущих задач. | `/model flash` или `/model claude-sonnet-4-6` |
| `/agent [имя]` | Переключить активного агента между agy и claude. | `/agent claude` |
| `/mode [mcp|cli|api]` | Переключить режим выполнения (MCP-каналы, локальный CLI или прямой API с файловыми инструментами). | `/mode mcp` |
| `/usage` (или `/limits`) | Проверить остаток бесплатных квот и платных кредитов Antigravity. | `/usage` |
| `/tokens` (или `/stats`) | Статистика токенов текущей задачи: Input, Output, Thinking, Cache Read и Hit Rate. | `/tokens` |
| `/context [id]` | Диаграмма заполнения окна контекста модели текущей или указанной задачи. | `/context` |
| `/top` (или `/ps`, `/resources`) | Мониторинг ресурсов сервера: нагрузка CPU, память RAM, свободное место на диске, Load Avg, cgroup, процессы `agy` и `claude`, а также `whisper-server`. | `/top` |

---

### ⚙️ Управление процессом бота

| Команда | Описание | Пример |
|---|---|---|
| `/restart` | Безопасный перезапуск службы бота через `systemctl`. | `/restart` |
| `/rebuild [ветка] [--pull] [--force]` | Обновление кода из Git, пересборка бинарника Go и перезапуск бота. | `/rebuild main --pull` |
| `/language` | Выбор языка интерфейса (кнопки строятся по установленным каталогам). | `/language` |
| `/script <имя>` | Запуск пользовательского скрипта из папки `SCRIPTS_DIR` и вывод результата. | `/script deploy.sh` |

---

## 🌍 Язык интерфейса и локализация

Любой текст, который видит пользователь, — ответы, инлайн-кнопки, описания команд в меню мессенджера, сообщения об ошибках и промпты, уходящие агенту, — берётся из каталога сообщений, а не зашит в код.

### Переключение языка

- `/language` показывает по кнопке на каждый установленный каталог; активный язык отмечен галочкой.
- Выбор сохраняется в SQLite в настройке `bot_language` и восстанавливается при следующем старте.
- Пока язык не выбран, бот отвечает на **английском** (`i18n.Default`). Незнакомый или удалённый код языка откатывается на английский, а не оставляет бота без языка.
- При смене языка меню подсказок мессенджера перерегистрируется для всех каталогов, поэтому подсказки по `/` идут вслед за интерфейсом.

Промпты агенту тоже переводятся: инструкции планирования, реализации и диалогового режима берутся из того же каталога, поэтому агент пишет планы и отчёты на выбранном языке.

### Как добавить новый язык

Добавление языка **не требует правок кода на Go** — достаточно положить один файл в `internal/i18n/locales/`:

1. Скопируйте `internal/i18n/locales/en.json` в `internal/i18n/locales/<код>.json`, где `<код>` — двухбуквенный код языка (`de`, `fr`, `es`, …). Имя файла и `meta.code` должны совпадать.
2. Заполните `meta`:

   ```json
   {
     "meta": { "code": "de", "name": "Deutsch", "flag": "🇩🇪" },
     "messages": { "...": "..." }
   }
   ```

   `name` — самоназвание языка, `flag` — эмодзи флага; вместе они становятся подписью кнопки в `/language`.
3. Переведите все значения в `messages`. Сохраните подстановки (`%s`, `%d`, `%.1f`) в том же порядке, что и в `en.json`, и не трогайте HTML-теги (`<b>`, `<code>`, `<a href="…">`).
4. Запустите тесты каталогов:

   ```bash
   go test ./internal/i18n/
   ```

   Они падают с перечнем проблем, если ключ забыт, придуман лишний ключ, которого нет в каталоге по умолчанию, или подстановки разошлись с английским оригиналом.
5. Пересоберите бота. Каталоги вшиваются через `go:embed`, поэтому новый язык сам появляется в `/language`, в меню команд и в цепочке отката.

Частично переведённый каталог остаётся рабочим: отсутствующий ключ откатывается на английский, а не ломает сообщение, — но тесты об этом скажут.

---

## 📋 Регламент работы агента (AGENTS.md)

В корне проектов репозиториев (включая этот проект) используется файл правил **`AGENTS.md`** (с символической ссылкой **`AGENTS.md`**):

1. **Режим планирования**:
   - При выполнении этапа планирования агент не создает ветки, не меняет файлы и формирует четкий пошаговый план для утверждения пользователем.
2. **Ветка разработки**:
   - Для каждой задачи создается ветка от актуального `main`: `feat/краткое-описание` или `fix/краткое-описание`.
3. **Разработка и качество**:
   - Изменения покрываются тестами, запускаются линтеры проекта (`go test ./...`, `go vet ./...`).
4. **Коммиты**:
   - Коммиты оформляются строго по стандарту **Conventional Commits** (`feat: ...`, `fix: ...`, `docs: ...`, `refactor: ...`).
5. **Pull Request и завершение**:
   - Ветка пушится в удаленный репозиторий: `git push -u origin HEAD`.
   - Создается PR через GitHub CLI: `gh pr create --fill`.
   - В конце своего финального ответа агент обязательно выводит строчку:
     ```text
     PR_URL: <полная ссылка на созданный PR>
     ```
   - Бот перехватывает этот маркер регулярным выражением и отправляет ссылку на готовый PR пользователю в чат!

> 💡 При клонировании новых проектов командой `/clone` бот автоматически проверяет наличие `AGENTS.md` и копирует его из корня `PROJECTS_ROOT`, если в репозитории его еще нет.

---

## 🧪 Разработка и тестирование

### Локальный запуск без systemd (режим разработки)

```bash
# Экспорт переменных из .env и запуск
set -a
source .env
set +a
go run ./cmd/bot
```

### Запуск тестов

Проект имеет развитое тестовое покрытие для всех модулей (`handlers`, `models`, `system`, `domain`, `utils`):

```bash
# Запуск всех тестов проекта
go test -v ./...

# Запуск статического анализатора кода
go vet ./...

# Проверка покрытия тестами
go test -cover ./...
```

---

## 🔒 Безопасность

1. **Строгая авторизация по Telegram ID**:
   - Middleware бота проверяет `c.Sender().ID == config.AdminID`.
   - Любые сообщения, команды или callback-запросы от сторонних пользователей отбрасываются без обработки.
2. **Безопасная работа с путями и именами**:
   - Функции `sanitizeProjectName` и проверка `filepath.Clean` защищают от атак обхода каталогов (Path Traversal `../`).
   - Имена проектов ограничены безопасным набором символов `[a-zA-Z0-9_\.\-]`.
3. **Защита от инъекций аргументов**:
   - Вызовы внешних утилит (`git`, `go`, `systemctl`, `agy`) выполняются напрямую через `exec.CommandContext` без использования небезопасной оболочки `sh -c`.
   - Аргументы URL и пути экранируются разделителем `--`.


---

## 📄 Лицензия

Проект распространяется под лицензией MIT. Подробности см. в файле [LICENSE](LICENSE).
