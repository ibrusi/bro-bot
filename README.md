...
- **Мониторинг квот, токенов и контекста**:
  ...
  - `/top` (или `/ps`, `/resources`) — мониторинг нагрузки хоста в реальном времени: CPU, RAM, Load Average, метрики cgroup службы systemd и процессы агентов `agy` и `claude`.
...
| `/top` (или `/ps`, `/resources`) | Мониторинг ресурсов сервера: нагрузка CPU, память RAM, Load Avg, cgroup и процессы `agy` и `claude`. | `/top` |
