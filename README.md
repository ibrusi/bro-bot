...
- **Мониторинг квот, токенов и контекста**:
  ...
  - `/top` (или `/ps`, `/resources`) — мониторинг нагрузки хоста в реальном времени: CPU, RAM, Load Average, метрики cgroup службы systemd и процессы `agy`.
...
| `PROJECTS_ROOT` | Нет | `/home/deploy/projects` | Абсолютный путь к каталогу, где хранятся репозитории рабочих проектов. |
| `DEFAULT_PROJECT` | Нет | `tg-bot-agent` | Имя проекта (подпапки в `PROJECTS_ROOT`), выбираемого активным при старте. |
| `DEFAULT_MODEL` | Нет | `gemini-3.1-pro-high` | Модель по умолчанию для `agy` (например `gemini-3.1-pro-high`, `flash`, `sonnet`, `opus`). |
| `QUESTION_TIMEOUT` | Нет | `15m` | Таймаут ожидания ответа на вопрос агента (`ask_question`). Поддерживает форматы `15m`, `300s`, `1h` или число секунд. По истечении задача ставится на паузу. |
| `BOT_DIR` | Нет | `/home/deploy/tg-agent-bot` | Путь к исходному коду бота для выполнения команд `/rebuild` и сохранения маркера перезапуска. |
| `BOT_SERVICE_NAME` | Нет | `tg-bot.service` | Имя службы systemd для перезапуска через команды `/restart` и `/rebuild`. |
...
| `/new [проект] <текст>` | Создать новую задачу явно (с возможностью указать проект отличный от текущего). | `/new my-service оптимизируй SQL запросы` |
| `/pause [id]` | Приостановить выполнение задачи (разблокирует очередь проекта для других задач). | `/pause 1` |
| `/resume [id] [ответ]` | Возобновить выполнение приостановленной задачи (можно сразу передать ответ на вопрос). | `/resume 1 используй второй вариант` |
| `/status [id]` | Подробный статус задачи: модель, затраченное время, последние логи, очередь правок. | `/status 2` |
...
| `/top` (или `/ps`, `/resources`) | Мониторинг ресурсов сервера: нагрузка CPU, память RAM, Load Avg, cgroup и процессы `agy`. | `/top` |
